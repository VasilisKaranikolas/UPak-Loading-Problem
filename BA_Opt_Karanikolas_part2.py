...
model = pyo.AbstractModel(name = "BA_Opt_Karanikolas_part2")
...
model.Bundle = pyo.Set(dimen=2)            #Bundle_set
model.Forbidden = pyo.Set(dimen=2)         #Forbidden_set
...
#periorismos Bundle
def bundle_rule(model, i, j):
    if (i, j) in model.Bundle:
        for z in model.T:
            if model.x[i, z] == 1 or model.x[j, z] == 1:
                return model.x[i, z]== 1 and model.x[j, z] == 1   
    else:
        return pyo.Constraint.Skip
model.bundle = pyo.Constraint(model.N, model.N, rule=bundle_rule)

#periorismos Forbidden
def forbidden_rule(model, i, j):
    if (i, j) in model.Forbidden:
        for z in model.T:
            return model.x[i, z] + model.x[j, z] <= 1
    else:
        return pyo.Constraint.Skip
model.forbidden = pyo.Constraint(model.N, model.N, rule=forbidden_rule)

#lush tou modelou me th xrhsh tou glpk solver
model = model.create_instance('BA_Opt_2023_part2.dat')
